...
func TestSettingHandler_GetPublicSettings_ExposesAnalyticsConfiguration(t *testing.T) {
	gin.SetMode(gin.TestMode)
	h := NewSettingHandler(service.NewSettingService(&settingHandlerPublicRepoStub{values: map[string]string{}}, &config.Config{
	...
	var resp struct {
		...
		Data struct {
			...
			PlausibleScriptURL        string `json:"plausible_script_url"`
			MicrosoftClarityProjectID string `json:"microsoft_clarity_project_id"`
		} `json:"data"`
	}
	require.NoError(t, json.Unmarshal(recorder.Body.Bytes(), &resp))
	require.Equal(t, 0, resp.Code)
	require.Equal(t, "sub2api.ai-baby-dance.com", resp.Data.PlausibleDomain)
	require.Equal(t, "https://plausible.ai-baby-dance.com/js/script.js", resp.Data.PlausibleScriptURL)
	require.Equal(t, "clarity-project", resp.Data.MicrosoftClarityProjectID)
}
